...
// export function ChatSheet({ contextActive }: { contextActive: boolean }) {
//   const [query, setQuery] = useState("");
//   const [messages, setMessages] = useState<{role: 'user' | 'ai', text: string, chunks?: Chunk[]}[]>([]);
//   const [loading, setLoading] = useState(false);
//   const scrollRef = useRef<HTMLDivElement>(null);
...
//       setMessages(prev => [...prev, { role: 'ai', text: data.answer, chunks: data.chunks }]);
//     } catch (error) {
//       setMessages(prev => [...prev, { role: 'ai', text: "I'm having trouble connecting to the medical core. Please ensure the backend is running." }]);
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       {/* 2. Ensure flex-col and h-full so scroll area knows its limits */}
//       <SheetContent className="w-full sm:w-[450px] flex flex-col p-0 h-full border-l shadow-2xl">
...
//             </div>
//           </SheetTitle>
//           {/* 3. Added Description to fix the Warning */}
//           <SheetDescription className="text-xs text-slate-400">
//             Ask questions about your uploaded report. AI responses are for informational purposes only.
//           </SheetDescription>
//         </SheetHeader>

//         {/* 4. ScrollArea needs flex-1 to take up remaining space and overflow-hidden to force scrollbars */}
//         <ScrollArea className="flex-1 p-5 bg-slate-50 overflow-y-auto">
//           {messages.length === 0 && (
//              <div className="h-full flex flex-col items-center justify-center text-center opacity-60 p-6 pt-20">
//                <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mb-4 text-blue-500">
...
//                <h4 className="font-bold text-slate-700 mb-1">How can I help?</h4>
//                <p className="text-xs text-slate-500 max-w-[200px]">
//                  Ask me about your "High Glucose" results or "What diet should I follow?"
//                </p>
//              </div>
//           )}
...
//                         <div className="flex gap-2 flex-wrap">
//                             {m.chunks.map((c, idx) => (
//                                 <Badge key={idx} variant="secondary" className="bg-slate-200/50 text-slate-500 text-[9px] hover:bg-slate-200 cursor-pointer">
//                                     Source: {c.test_name || "Page " + (idx+1)}
//                                 </Badge>
//                             ))}
//                         </div>
...
//           <div className="relative">
//             <Input 
//                 className="pr-12 py-6 rounded-full border-slate-200 bg-slate-50 focus-visible:ring-blue-500 focus-visible:ring-offset-0"
...
//             <Button 
//                 size="icon" 
//                 className="absolute right-1.5 top-1.5 h-9 w-9 rounded-full bg-blue-600 hover:bg-blue-700 transition-colors"
//                 onClick={handleSend} 
//                 disabled={loading}
//             >
//                 <Send size={16} />
//             </Button>
//           </div>
//           <p className="text-[10px] text-center text-slate-300 mt-2">AI can make mistakes. Verify with a doctor.</p>
//         </div>
//       </SheetContent>
//     </Sheet>
//   );
