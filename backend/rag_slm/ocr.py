...
def main():
    ...
    output["merged_full_text"] = "\n\n--- PAGE BREAK ---\n\n".join(p["full_text"] for p in output["pages"])
    output["merged_rows"] = sum((p["rows"] for p in output["pages"]), [])
    # output["created_at"] = datetime.utcnow()
    output["status"] = "OCR_COMPLETED"

    # 2. NOW run make_json_safe so the 'created_at' datetime becomes a string
    safe_output = make_json_safe(output)

    # 3. Save to MongoDB
    ocr_doc_id = save_ocr_json(safe_output)
    
    # 4. Inject the ID as a string into the safe dictionary
    safe_output["ocr_doc_id"] = str(ocr_doc_id)

    # 5. Finally, write the safe dictionary to the local file
