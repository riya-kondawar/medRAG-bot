...
def main():
    ...
    if args.mongo:
        ...
        for r in reports:
            ...
            # 🔥 FALLBACK: use raw OCR text if structured extraction failed
            if not docs:
                print(f"⚠️ No structured data found for {r['_id']}, falling back to raw OCR text")
                raw_text = r.get("merged_full_text", "")
                if raw_text.strip():
                    docs = [
                        Document(
    ...
    if not chunks:
        raise RuntimeError(
            "❌ Ingestion produced 0 chunks. Check OCR output or build_documents()."
        )
