...
def build_documents(report: Dict[str, Any], report_id: Optional[str] = None) -> List[Document]:
    ...
    Produces:
      - 1 summary document
      - 1 document per test
    Returns an empty list if no meaningful data is found.
    """
    # 1. Improved source identification
    source = report_id or report.get("source") or report.get("ocr_doc_id") or "unknown_source"
    report_type = report.get("report_type") or "UNKNOWN"

    patient = report.get("patient_info") or {}
    pblock = patient_block(patient)

    # 2. Extract and Normalize tests
    # If using OCR output directly, tests might be in 'merged_rows' or 'pages'
    # Ensure your ingest.py/parse.py passes the correct 'tests' list here
    tests_raw = report.get("tests") or []
    
    # CRITICAL GUARDRAIL: If no tests and no patient name, don't index anything
    patient_name = patient.get("name", "").lower()
    if not tests_raw and (not patient_name or patient_name == "unknown"):
        print(f"⚠️ Skipping ingestion for {source}: No tests or patient info found.")
        return []
    ...
    # ---------- Summary doc (Only if there is actually data) ----------
    summary_lines = [
        f"REPORT TYPE: {report_type}",
        pblock,
        f"TOTAL TESTS: {len(enriched_tests)}",
    ...
    if enriched_tests or (patient_name and patient_name != "unknown"):
        docs.append(
            Document(
                page_content="\n".join([x for x in summary_lines if x.strip()]),
                metadata={
